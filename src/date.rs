...
#[derive(PartialEq, Eq, Clone, Copy)]
pub struct DayOfTheMonth(pub u16);
...
impl DayOfTheMonth {
    pub fn new(day_of_the_month: u16) -> DayOfTheMonth {
...
pub fn days_in_month(year: Year, month: Month) -> impl Iterator<Item = DayOfTheMonth> {
    ...
    let next_date = NaiveDate::from_ymd_opt(year_for_next_date, month_for_next_date, 1)
        .expect("Error creating next date.");

    let original_date =
        NaiveDate::from_ymd_opt(year.0, month.0, 1).expect("Error creating original date.");

    let num_days = next_date.signed_duration_since(original_date).num_days() as u16;

    (1..=num_days).map(DayOfTheMonth::new)
}

const valid_months: RangeInclusive<u32> = 1..=12;
const valid_days_of_the_month: RangeInclusive<u16> = 1..=31;
